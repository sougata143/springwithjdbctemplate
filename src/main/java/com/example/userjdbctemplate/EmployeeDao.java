...
//			jt.execute(sql);
	...
	public List<Employee> getAllEmployeeRestWithDepartment(String firstname){
		JdbcTemplate jt = new JdbcTemplate(dataSource);
		String sql = "select emp.id, emp.firstname, emp.lastname, emp.email, emp.address, emp.contact, dept.dept_name, emp.gender, emp.department, dept.dept_name"
				+ " from employee emp, department dept where emp.firstname = ?";
	...
	@Transactional
	public List<Employee> getAllEmployeeRest1(){
		JdbcTemplate jt = new JdbcTemplate(dataSource);
		//String sql = "select * from employee";
		String sql = "select emp.id, emp.firstname, emp.lastname, emp.email, "
				+ "emp.address, emp.contact, emp.gender, emp.department from employee emp";
		String sql2 = "select dept.dept_name from department dept";
		Collection<? extends Employee> departments = jt.query(sql2, new BeanPropertyRowMapper(Department.class));
		//String deptname = departments.get(0).getDeptname();
		
		List<Employee> employees = jt.query(sql, new BeanPropertyRowMapper(Employee.class));
		employees.addAll(employees.size(), departments);
	...
	public String saveTableDataBody(Employee employee) {
		...
		String sql = "insert into employee (id, firstname, lastname, address, contact, email, gender, department) "
				+ "values ('"+employee.getId()+"','"+employee.getFirstname()+"','"+employee.getLastname()+"','"+employee.getAddress()+"','"+employee.getContact()+"','"+employee.getEmail()+"','"+employee.getGender()+"','"+employee.getDepartment()+"')";
		try {
//			jt.execute(sql);
			jt.execute(sql);
			System.out.println("data inserted successfully");
			System.out.println("data inserted "+employee.getFirstname()+" "+employee.getLastname()+" "+employee.getAddress()+" "+employee.getContact()+" "+employee.getEmail()+" "+employee.getGender());
		}catch(Exception e) {
			e.printStackTrace();
		}
		
		return ("data inserted "+employee.getFirstname()+" "+employee.getLastname()+" "+employee.getAddress()+" "+employee.getContact()+" "+employee.getEmail()+" "+employee.getGender());
	}
